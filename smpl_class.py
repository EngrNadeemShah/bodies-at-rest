...
class SMPLPreloader:
	def __init__(self, smpl_male, smpl_feml, device, config):
		...
		self.shapedirs_m_orig	= smpl_male.shapedirs.permute(2, 0, 1).to(device)	# (10, 6890, 3)
		self.v_template_f_orig	= smpl_feml.v_template.to(device)	# (6890, 3)
		self.v_template_m_orig	= smpl_male.v_template.to(device)	# (6890, 3)
		self.J_regressor_f_orig	= smpl_feml.J_regressor.to(device)	# (24, 6890)
		self.J_regressor_m_orig	= smpl_male.J_regressor.to(device)	# (24, 6890)

		# Select the 10 vertices of interest for posedirs and weights
		if config['mod'] == 1:
			self.posedirs_f_orig= smpl_feml.posedirs.reshape(207, 6890, 3).index_select(1, torch.tensor(self.vertices, device=self.device))	# (207, 10, 3)
			self.posedirs_m_orig= smpl_male.posedirs.reshape(207, 6890, 3).index_select(1, torch.tensor(self.vertices, device=self.device))	# (207, 10, 3)
			self.weights_f_orig	= smpl_feml.lbs_weights.index_select(0, torch.tensor(self.vertices, device=self.device))	# (10, 24)
			self.weights_m_orig	= smpl_male.lbs_weights.index_select(0, torch.tensor(self.vertices, device=self.device))	# (10, 24)
		elif config['mod'] == 2:
	...
	def expand_params(self):
		...
		self.shapedirs_f = self.shapedirs_f_orig.unsqueeze(0).expand(self.batch_size, -1, -1, -1)	# (B, 10, 6890, 3)
		self.shapedirs_m = self.shapedirs_m_orig.unsqueeze(0).expand(self.batch_size, -1, -1, -1)	# (B, 10, 6890, 3)
		
		self.v_template_f = self.v_template_f_orig.unsqueeze(0).expand(self.batch_size, -1, -1)		# (B, 6890, 3)
		self.v_template_m = self.v_template_m_orig.unsqueeze(0).expand(self.batch_size, -1, -1)		# (B, 6890, 3)
		
		self.J_regressor_f = self.J_regressor_f_orig.unsqueeze(0).expand(self.batch_size, -1, -1)	# (B, 24, 6890)
		self.J_regressor_m = self.J_regressor_m_orig.unsqueeze(0).expand(self.batch_size, -1, -1)	# (B, 24, 6890)
		
		self.posedirs_f = self.posedirs_f_orig.unsqueeze(0).expand(self.batch_size, -1, -1, -1)		# (B, 207, 10, 3)
		self.posedirs_m = self.posedirs_m_orig.unsqueeze(0).expand(self.batch_size, -1, -1, -1)		# (B, 207, 10, 3)

		self.weights_f = self.weights_f_orig.unsqueeze(0).expand(self.batch_size, -1, -1)			# (B, 10, 24)
		self.weights_m = self.weights_m_orig.unsqueeze(0).expand(self.batch_size, -1, -1)			# (B, 10, 24)
	...
	def fetch_gender_based_parameters(self, gender_labels):
		...
		self.batch_size = gender_labels.shape[0]
		self.expand_params()
