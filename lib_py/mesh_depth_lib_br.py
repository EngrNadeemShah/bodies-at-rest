...
class MeshDepthLib():
	...
	def __init__(self, loss_type, batch_size, vertices):
		...
		if vertices == "all":   # mod 2
			...
			self.J_regressor = torch.cat((self.SMPL_J_regressor_f.unsqueeze(0).repeat(self.batch_size, 1, 1).unsqueeze(0),
											self.SMPL_J_regressor_m.unsqueeze(0).repeat(self.batch_size, 1, 1).unsqueeze(0)), 0)  # this is 2 x N x R x 24
			...
			self.SMPL_posedirs_f = torch.Tensor(np.stack([SMPL_model_female.posedirs[vertices[0], :, :],
													...
													SMPL_model_female.posedirs[vertices[3], :, :],
													SMPL_model_female.posedirs[vertices[4], :, :],
													SMPL_model_female.posedirs[vertices[5], :, :],
													SMPL_model_female.posedirs[vertices[6], :, :],
													SMPL_model_female.posedirs[vertices[7], :, :],
													SMPL_model_female.posedirs[vertices[8], :, :],
													SMPL_model_female.posedirs[vertices[9], :, :]]))

			self.SMPL_weights_f = torch.Tensor(np.stack([SMPL_model_female.weights[vertices[0], :],
													SMPL_model_female.weights[vertices[1], :],
													SMPL_model_female.weights[vertices[2], :],
													SMPL_model_female.weights[vertices[3], :],
													SMPL_model_female.weights[vertices[4], :],
													SMPL_model_female.weights[vertices[5], :],
													SMPL_model_female.weights[vertices[6], :],
			...
			self.SMPL_shapedirs_repeat_f = self.SMPL_shapedirs_f.unsqueeze(0).repeat(self.batch_size, 1, 1, 1).permute(0, 2, 1, 3).unsqueeze(0)
			self.SMPL_shapedirs_repeat_m = self.SMPL_shapedirs_m.unsqueeze(0).repeat(self.batch_size, 1, 1, 1).permute(0, 2, 1, 3).unsqueeze(0)
			self.SMPL_shapedirs_repeat = torch.cat((self.SMPL_shapedirs_repeat_f, self.SMPL_shapedirs_repeat_m), 0)  # this is 2 x N x B x R x D
			...
			self.SMPL_shapedirs_repeat = self.SMPL_shapedirs_repeat.permute(1, 0, 2, 3, 4).view(self.batch_size, 2, self.SMPL_B * self.SMPL_R * self.SMPL_D)
			...
			self.SMPL_v_template_repeat = torch.cat((self.SMPL_v_template_repeat_f, self.SMPL_v_template_repeat_m), 0)  # this is 2 x N x R x D
			...
			self.SMPL_J_regressor_repeat = torch.cat((self.SMPL_J_regressor_repeat_f, self.SMPL_J_regressor_repeat_m), 0)  # this is 2 x N x R x 24
	...
	def HMR(self, gender_switch, betas_est, Rs_est, root_shift_est, start_incr, end_incr, GPU):
		...
		shapedirs = torch.bmm(gender_switch[start_incr:end_incr, :, :],
							  self.shapedirs[0:sub_batch_size, :, :]).view(sub_batch_size, self.SMPL_B, self.SMPL_R * self.SMPL_D)

		betas_shapedirs_mult = torch.bmm(betas_est[start_incr:end_incr, :].unsqueeze(1), shapedirs).squeeze(1).view(sub_batch_size, self.SMPL_R, self.SMPL_D)

		v_template = torch.bmm(gender_switch[start_incr:end_incr, :, :], self.v_template[0:sub_batch_size, :, :]).view(sub_batch_size, self.SMPL_R, self.SMPL_D)

		v_shaped = betas_shapedirs_mult + v_template

		J_regressor = torch.bmm(gender_switch[start_incr:end_incr, :, :], self.J_regressor[0:sub_batch_size, :, :]).view(sub_batch_size, self.SMPL_R, 24)
		...
		targets_est, A_est = KinematicsLib().batch_global_rigid_transformation(Rs_est[start_incr:end_incr, :], J_est, self.parents, GPU, rotate_base=False)
		...
		posedirs = torch.bmm(gender_switch[start_incr:end_incr, :, :], self.posedirs[0:sub_batch_size, :, :]).view(sub_batch_size, self.R_used * self.SMPL_D, 207).permute(0, 2, 1)

		v_posed = torch.bmm(pose_feature.unsqueeze(1), posedirs).view(-1, self.R_used, self.SMPL_D)

		v_posed = v_posed.clone() + v_shaped

		weights_repeat = torch.bmm(gender_switch[start_incr:end_incr, :, :], self.SMPL_weights_repeat[0:sub_batch_size, :, :]).squeeze(1).view(sub_batch_size, self.R_used, 24)
	...
	def PMR(self, verts, bed_angle_batch, get_mesh_bottom_dist = True):
		...
		vertice_sorting_method = (verts_taxel_int[:, :, 0:1] + 1) * 10000000 + (verts_taxel_int[:, :, 1:2] + 1) * 100000 + verts_taxel_int[:, :, 2:3]
