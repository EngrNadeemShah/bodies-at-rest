...
class CNN(nn.Module):
	...
	def forward_kinematic_angles(self, inputs, label_gender_switch, label_synth_real_switch, config, OUTPUT_EST_DICT,
								 label_markers_xyz=None, is_training=True, label_body_shape=None, label_joint_angles=None, label_root_xyz=None):

		OUTPUT_DICT = {}

		if config['first_pass']:
			self.vertices = "all" if config['depth_map_output'] else [1325, 336, 1032, 4515, 1374, 4848, 1739, 5209, 1960, 5423]
			self.SMPL_meshDepthLib = MeshDepthLib(loss_type=self.loss_type, batch_size=inputs.size(0), vertices=self.vertices)

		if config['all_tanh_activ']:
			predicted_labels = self.CNN_packtanh_double(inputs) if config['double_network_size'] else self.CNN_packtanh(inputs)
		else:
			predicted_labels = self.CNN_pack1(inputs)

		predicted_labels = predicted_labels.reshape(predicted_labels.shape[0], -1)

		if config['double_network_size']:
		...
		if config['lock_root']:
			predicted_labels[:, 10:13] += torch.tensor([0.6, 1.2, 0.1], device=predicted_labels.device).detach()
		elif not config['adjust_ang_from_est']:
			predicted_labels[:, 10:13] += torch.tensor([0.6, 1.2, 0.1], device=predicted_labels.device)

		if config['full_body_rot']:
			predicted_labels = F.pad(predicted_labels, (0, 3))
			if config['adjust_ang_from_est']:
				predicted_labels[:, 13:19] += OUTPUT_EST_DICT['root_atan2_est']
		...
		if config['adjust_ang_from_est']:
			predicted_labels[:, 0:10]		+= OUTPUT_EST_DICT['betas_est']
			predicted_labels[:, 10:13]		+= OUTPUT_EST_DICT['root_xyz_est']
			if config['full_body_rot']:
				predicted_labels[:, 22:91]	+= OUTPUT_EST_DICT['angles_est'][:, 3:72]
			else:
				predicted_labels[:, 13:85]	+= OUTPUT_EST_DICT['angles_est']

		OUTPUT_DICT.update({
			'y_pred_betas':		predicted_labels[:, 0:10].clone().data,
			'y_pred_root_xyz':	predicted_labels[:, 10:13].clone().data,
			'y_pred_root_atan2':predicted_labels[:, 13:19].clone().data if config['full_body_rot'] else None,
			'y_pred_angles':	predicted_labels[:, 13+OSA:85+OSA].clone().data
		})

		add_idx = 72 if config['regress_angles'] else 0

		if config['clip_betas']:
			predicted_labels[:, 0:10] = torch.tanh(predicted_labels[:, 0:10] / 3) * 3

		test_ground_truth = False	# Set to True only if the dataset is entirely synthetic and anglesDC is used
		if not is_training or not test_ground_truth:
			y_pred_betas	= predicted_labels[:, 0:10].clone()
			y_pred_root_xyz	= predicted_labels[:, 10:13].clone()

			# Normalize for tanh activation function
			bounds_mean = torch.mean(self.SMPL_meshDepthLib.bounds[0:72, 0:2], dim=1)
			bounds_diff = torch.abs(self.SMPL_meshDepthLib.bounds[0:72, 0] - self.SMPL_meshDepthLib.bounds[0:72, 1])
			scaled_labels = (predicted_labels[:, 13+OSA:85+OSA] - bounds_mean) * (2.0 / bounds_diff)
			tanh_labels = torch.tanh(scaled_labels)
			predicted_labels[:, 13+OSA:85+OSA] = tanh_labels / (2.0 / bounds_diff) + bounds_mean

			config['align_procr'] = False
			if config['align_procr']:
				y_pred_root_xyz = label_root_xyz
				predicted_labels[:, 13+OSA:16+OSA] = label_joint_angles[:, 0:3].clone()
			if self.loss_type == 'anglesDC':
				y_pred_angles_rot_mat = KinematicsLib().batch_rodrigues(predicted_labels[:, 13+OSA:85+OSA].view(-1, 24, 3)).view(-1, 24, 3, 3)
			elif self.loss_type == 'anglesEU':
				y_pred_angles_rot_mat = KinematicsLib().batch_euler_to_R(predicted_labels[:, 13+OSA:85+OSA].view(-1, 24, 3), self.SMPL_meshDepthLib.zeros_cartesian, self.SMPL_meshDepthLib.ones_cartesian).view(-1, 24, 3, 3)
		else:
			y_pred_betas = label_body_shape
			predicted_labels[:, 0:10] = label_body_shape.clone()
			predicted_labels[:, 13+OSA:85+OSA] = label_joint_angles.clone()
			y_pred_root_xyz = label_root_xyz
			if self.loss_type == 'anglesDC':
				y_pred_angles_rot_mat = KinematicsLib().batch_rodrigues(predicted_labels[:, 13+OSA:85+OSA].view(-1, 24, 3)).view(-1, 24, 3, 3)

		OUTPUT_DICT.update({
			'y_pred_betas_post_clip':	predicted_labels[:, 0:10].clone().data,
			'y_pred_angles_post_clip': (
				KinematicsLib().batch_dir_cos_angles_from_euler_angles(
					predicted_labels[:, 13+OSA:85+OSA].view(-1, 24, 3), 
					self.SMPL_meshDepthLib.zeros_cartesian, 
					self.SMPL_meshDepthLib.ones_cartesian
				) if self.loss_type == 'anglesEU' else predicted_labels[:, 13+OSA:85+OSA].view(-1, 24, 3).clone() if self.loss_type == 'anglesDC' else None
			),
			'y_pred_root_xyz_post_clip': predicted_labels[:, 10:13].clone().data
		})

		label_gender_switch = label_gender_switch.unsqueeze(1)
		current_batch_size = label_gender_switch.size(0)
		...
		if config['depth_map_output'] == True:
			...
			if current_batch_size % self.SMPL_meshDepthLib.batch_size != 0:
				sub_batch_sizes = num_normal_sub_batches * [self.SMPL_meshDepthLib.batch_size] + [current_batch_size % self.SMPL_meshDepthLib.batch_size]
			else:
				sub_batch_sizes = num_normal_sub_batches * [self.SMPL_meshDepthLib.batch_size]

			start_incr, end_incr = 0, 0


			for sub_batch_size in sub_batch_sizes:
				end_incr += sub_batch_size
				verts_sub, J_est_sub, targets_est_sub = self.SMPL_meshDepthLib.HMR(label_gender_switch, y_pred_betas,
				...
				start_incr += sub_batch_size

			bed_ang_idx = -1

			if config['incl_ht_wt_channels'] == True: bed_ang_idx -= 2

			bed_angle_batch = torch.mean(inputs[:, bed_ang_idx, 1:3, 0], dim=1)
			...
			SMPL_shapedirs = torch.bmm(label_gender_switch, self.SMPL_meshDepthLib.SMPL_shapedirs_repeat[0:current_batch_size, :, :])\
							.view(current_batch_size, self.SMPL_meshDepthLib.SMPL_B, self.SMPL_meshDepthLib.SMPL_R*self.SMPL_meshDepthLib.SMPL_D)

			SMPL_shapedirs_y_pred_betas_mult = torch.bmm(y_pred_betas.unsqueeze(1), SMPL_shapedirs).squeeze(1)\
							.view(current_batch_size, self.SMPL_meshDepthLib.SMPL_R, self.SMPL_meshDepthLib.SMPL_D)

			SMPL_v_template = torch.bmm(label_gender_switch, self.SMPL_meshDepthLib.SMPL_v_template_repeat[0:current_batch_size, :, :])\
							.view(current_batch_size, self.SMPL_meshDepthLib.SMPL_R, self.SMPL_meshDepthLib.SMPL_D)

			SMPL_pred_v_shaped = SMPL_shapedirs_y_pred_betas_mult + SMPL_v_template

			SMPL_J_regressor = torch.bmm(label_gender_switch, self.SMPL_meshDepthLib.SMPL_J_regressor_repeat[0:current_batch_size, :, :])\
			...
			y_pred_markers_xyz, SMPL_pred_A = KinematicsLib().batch_global_rigid_transformation(
				y_pred_angles_rot_mat, SMPL_pred_J, self.SMPL_meshDepthLib.parents, self.GPU, rotate_base=False)
			...
			SMPL_posedirs = torch.bmm(label_gender_switch, self.SMPL_meshDepthLib.SMPL_posedirs_repeat[0:current_batch_size, :, :]) \
							.view(current_batch_size, 10 * self.SMPL_meshDepthLib.SMPL_D, 207).permute(0, 2, 1)
			...
			SMPL_weights = torch.bmm(label_gender_switch, self.SMPL_meshDepthLib.SMPL_weights_repeat[0:current_batch_size, :, :]).squeeze(1).view(current_batch_size, 10, 24)
		...
		# if is_training == True:
		y_true_synth_real_switch_repeated = label_synth_real_switch.unsqueeze(1).repeat(1, 3)
		...
		predicted_labels[:, 0:10] = torch.mul(label_synth_real_switch.unsqueeze(1), torch.sub(predicted_labels[:, 0:10], label_body_shape))#*.2

		if config['full_body_rot'] == True:
			predicted_labels[:, 10:16] = predicted_labels[:, 13:19].clone()

			if self.loss_type == 'anglesEU':
				predicted_labels[:, 10:13] = predicted_labels[:, 10:13].clone() - torch.cos(KinematicsLib().batch_euler_angles_from_dir_cos_angles(label_joint_angles[:, 0:3].view(-1, 1, 3).clone()).contiguous().view(-1, 3))
				predicted_labels[:, 13:16] = predicted_labels[:, 13:16].clone() - torch.sin(KinematicsLib().batch_euler_angles_from_dir_cos_angles(label_joint_angles[:, 0:3].view(-1, 1, 3).clone()).contiguous().view(-1, 3))
			elif self.loss_type == 'anglesDC':
				predicted_labels[:, 10:13] = predicted_labels[:, 10:13].clone() - torch.cos(label_joint_angles[:, 0:3].clone())
				predicted_labels[:, 13:16] = predicted_labels[:, 13:16].clone() - torch.sin(label_joint_angles[:, 0:3].clone())


		# compare the output angles to the target values
		if config['regress_angles'] == True:
			if self.loss_type == 'anglesDC':
				predicted_labels[:, 34+OSA: 106+OSA] = label_joint_angles.clone().view(-1, 72) - predicted_labels[:, 13+OSA: 85+OSA]
				predicted_labels[:, 34+OSA: 106+OSA] = torch.mul(label_synth_real_switch.unsqueeze(1), torch.sub(predicted_labels[:, 34+OSA: 106+OSA], label_joint_angles.clone().view(-1, 72)))

			elif self.loss_type == 'anglesEU':
				predicted_labels[:, 34+OSA: 106+OSA] = KinematicsLib().batch_euler_angles_from_dir_cos_angles(label_joint_angles.view(-1, 24, 3).clone()).contiguous().view(-1, 72) - predicted_labels[:, 13+OSA: 85+OSA]

			predicted_labels[:, 34+OSA: 106+OSA] = torch.mul(label_synth_real_switch.unsqueeze(1), predicted_labels[:, 34+OSA: 106+OSA].clone())


		# compare the output joints to the target values
		predicted_labels[:, 34+add_idx+OSA: 106+add_idx+OSA] = label_markers_xyz[:, 0:72]/1000 - predicted_labels[:, 34+add_idx+OSA:106+add_idx+OSA]
		...
		for joint_num in range(24):
			if joint_num in [0, 1, 2, 6, 9, 10, 11, 12, 13, 14, 16, 17, 22, 23]: #torso is 3 but forget training it
				predicted_labels[:, 10+joint_num+OSA] = torch.mul(label_synth_real_switch,
